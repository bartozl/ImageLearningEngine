...
def get_dummy_input(resize):
    img_orig = transforms.ToTensor()(cv2.cvtColor(cv2.imread('../samples/dog.jpg'), cv2.COLOR_BGR2RGB))
    img_curr = add_pad(img_orig, resize=resize)
    return img_curr, img_orig

def get_input(cap, resize):
    ret, frame = cap.read()
